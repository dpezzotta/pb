...
// $clid = $_GET['clid'];
$cohort_id_selected = $_GET['cohort_id'];
$date = $_GET['date'];
$cohort_id = $_GET['cohort_id'];
$admin = new Admin();
$teacher = new Teacher();
// $students = $admin->get_awards_by_clid($clid, 0, $cohort_id_selected);
$students = $teacher->get_cohort_awards_by_date($date, $cohort_id_selected);
...
// 	$summer_badge_path = $admin->get_student_summer_badge_path($student_id);
	...
	$course_teachers = Award::course_teachers($p['course_location_id'], $p);
...
// 	  $this->Cell(0,4,'visit PlanetBravo.com/badges/'.$badge_username.' to view your accomplishments',0,1,'C',0,$badge_path);
	...
	$this->Ln(10);
	$this->SetFont('Arial','B',12);
	$this->Cell(0,6,'PlanetBravo @Home Camp',0,0,'C');
...
//	  $this->Image('http://www.PlanetBravo.com/badge/phpqrcode/showimage.php?data='.$badge_username.'.png',237,120,30,30);
  ...
  function Footer() {
	// 	date('F d, Y',$_REQUEST["course_end_date"]);	
	$day = strftime("%e",strtotime($_REQUEST["course_end_date"]));
	if ($day == '1') $th = "st";
	elseif ($day == '2') $th = "nd";
	elseif ($day == '3') $th = "rd";
	elseif ($day == '31') $th = "st";
	else $th = "th";
	$date = strftime("%B %d$th, %Y",strtotime($_REQUEST["course_end_date"]));
	$this->SetY(-40);
	$this->SetFont('Arial','I',12);
...
$file = "tmp_pdf/awards_$cohort_id"."_"."$date.pdf";
$pdf->Output($file);
